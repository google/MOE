...
public class SystemFileSystemTest extends TestCase {
  ...
  public void testCleanUpTempDirs() throws Exception {
    FileSystem fs = new SystemFileSystem();
    File tempDir1 = fs.getTemporaryDirectory("tempDir1");
    File tempDir2 = fs.getTemporaryDirectory("tempDir2");
    Files.touch(tempDir1);
    Files.touch(tempDir2);
    fs.cleanUpTempDirs();
    assertFalse("tempDir1", tempDir1.exists());
    assertFalse("tempDir2", tempDir2.exists());
  }

  public void testMarkAsPersistent() throws Exception {
    FileSystem fs = new SystemFileSystem();
    File tempDir = fs.getTemporaryDirectory("tempDir");
    File persistentDir = fs.getTemporaryDirectory("persistentDir");
    fs.markAsPersistent(persistentDir);
    Files.touch(tempDir);
    Files.touch(persistentDir);
    fs.cleanUpTempDirs();
    assertFalse("tempDir", tempDir.exists());
    assertTrue("persistentDir", persistentDir.exists());
  }
}
